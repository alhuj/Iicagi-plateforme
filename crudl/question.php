...
<BR>";
		...
		public function mes_sujets(){
			echo "<table class='table table-striped table-expansed table-radius'>
	    <caption class='text-center'>Liste de vos qestions</caption>
	    <thead>
	        <tr class='active'>
	            <th style='width:35%'>Sujet</th>
				<th style='width:5%'>Vus</th>
	...
	$resutat = mysql_query("SELECT * FROM question WHERE idUti=$idses") or die(mysql_error());
	$nbrTotalQues = mysql_num_rows($resutat);
	$nbrQuesParPage=10;
	$pagestr='<a href="mes_sujets.php?page=';
	...
	$req=mysql_query("SELECT pseudo, avatar, idQu, sujet, DATE_FORMAT(heureQu, '%d/%m/%Y à %Hh:%i') heureQu, resolu, q.idUti aut, idGrp
		FROM question AS q, utilisateur AS u
		where u.idUti=q.idUti AND q.idUti=$idses
		ORDER BY idQu DESC $limit");
	...
	$resutat = mysql_query("SELECT * FROM question Q, poste p
													WHERE idGrp=1 AND q.idQu=p.idQu
													GROUP BY q.idQu") or die(mysql_error());
	$nbrTotalQues = mysql_num_rows($resutat);
	$nbrQuesParPage=5;
	$pagestr='<a href="most_seen.php?page=';
	...
	$req=mysql_query("SELECT pseudo, avatar, q.idQu, sujet, DATE_FORMAT(heureQu, '%d/%m/%Y à %Hh:%i') heureQu, resolu, q.idUti aut, idGrp, count(v.idQu) nbVu
										FROM question AS q, utilisateur AS u, vu_question AS v
										where u.idUti=q.idUti AND idGrp=1 AND q.idQu=v.idQu
										GROUP BY q.idQu
										ORDER BY nbVu DESC $limit") or die(mysql_error());
	...
	$req=mysql_query("SELECT pseudo, avatar, q.idQu, sujet, DATE_FORMAT(heureQu, '%d/%m/%Y à %Hh:%i') heureQu, resolu, q.idUti aut, idGrp, count(p.idQu) nbPo
										FROM question AS q, utilisateur AS u, poste p
										where u.idUti=q.idUti AND idGrp=1 AND q.idQu=p.idQu
										GROUP BY p.idQu
										ORDER BY nbPo DESC limit 10") or die(mysql_error());
