...
while($affich = mysql_fetch_array($req)){
	...
	$show=mysql_fetch_array($result3);
					  ...
					  echo	"<tr>
					  		<th class='text-capitalize'><a href='http://localhost/plateforme/public/profile.php?idUti=".$affich['aut']."'>
						...
						echo"' style='display:block; height:60px; width:60px;' class='img-responsive profile-image img-rounded''>
					  		...
					  		<th><a href='http://localhost/plateforme/public/forum/voir_poste.php?idQu=".$affich['idQu']."'>".$affich['sujet']."</a></th>
							<th>".$show['vu']."</th>
	...
	if($affich['resolu']==1){
					  echo"<th class='text-success'>OUI</th>";
					   ...
					   echo"<th class='text-danger'>NON</th>";
						...
						echo "<th><button onclick='btnSupp(".$affich['idQu'].")' class='btn btn-danger'>Supprimer</button></th>";
}
					echo "	</tr>
							</tbody>
							</table>
...
public function supprimer(){
	...
	if(isset($_GET['id'])){
						$req = mysql_query("DELETE FROM `question` WHERE `idQu` = ".$_GET['id']) or die(mysql_error());
...
public function affiche(){
	$idQu=$_GET['idQu'];		
$sql="SELECT pseudo, sujet, detailQu, heureQu, q.idUti AS createur, resolu
